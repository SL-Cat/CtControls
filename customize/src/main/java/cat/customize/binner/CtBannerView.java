...
import cat.customize.binner.control.BannerPagerAdapter;
import cat.customize.binner.control.BannerViewPager;
import cat.customize.binner.control.ViewPagerScroller;
import cat.customize.binner.holder.BannerHolderCreator;
import cat.customize.binner.holder.BannerPageClickListener;
import cat.customize.binner.transformer.CoverPagerTransformer;
import cat.customize.binner.transformer.ScaleYPagerTransformer;
...
public class CtBannerView<T> extends RelativeLayout {
    ...
    private int[] mIndicatorRes = new int[]{R.drawable.ct_indicator_normal, R.drawable.ct_indicator_selected};
    ...
    private void readAttrs(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.CtBannerView);
        mIsOpen3DEffect = typedArray.getBoolean(R.styleable.CtBannerView_open_radio_mode, true);
        mIsMiddlePageCover = typedArray.getBoolean(R.styleable.CtBannerView_middle_page_cover, true);
        mIsCanLoop = typedArray.getBoolean(R.styleable.CtBannerView_canLoop, true);
        mIndicatorAlign = typedArray.getInt(R.styleable.CtBannerView_indicatorAlign, 1);
        mIndicatorPaddingLeft = typedArray.getDimensionPixelSize(R.styleable.CtBannerView_indicatorPaddingLeft, 0);
        mIndicatorPaddingRight = typedArray.getDimensionPixelSize(R.styleable.CtBannerView_indicatorPaddingRight, 0);
        mIndicatorPaddingTop = typedArray.getDimensionPixelSize(R.styleable.CtBannerView_indicatorPaddingTop, 0);
        mIndicatorPaddingBottom = typedArray.getDimensionPixelSize(R.styleable.CtBannerView_indicatorPaddingBottom, 0);
    }

    private void init() {
        View view = null;
        if (mIsOpen3DEffect) {
            view = LayoutInflater.from(getContext()).inflate(R.layout.ct_banner_radio_layout, this, true);
        } else {
            view = LayoutInflater.from(getContext()).inflate(R.layout.ct_banner_normal_layout, this, true);
        }
...
//        m3DModePadding = dpToPx(30);
    ...
    /**
     * 开始轮播
     * <p>应该确保在调用用了{@link CtBannerView {@link #setPages(List, BannerHolderCreator)}} 之后调用这个方法开始轮播</p>
     */
    public void start() {
     ...
     * @param holderCreator ViewHolder生成器 {@link BannerHolderCreator} And {@link }
     */
